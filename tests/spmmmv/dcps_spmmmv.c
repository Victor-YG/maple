...
volatile static uint32_t spmm_produce_cnt;
volatile static uint32_t spmm_consume_cnt;
...
// spmv globals
volatile static uint32_t spmv_produce_cnt;
volatile static uint32_t spmv_consume_cnt;
...
void spmm_access_init(uint32_t id) {
    A_nrows = A_shape[0];
    B_ncols = B_shape[1];
    C_indptr[0] = 0;
    spmm_produce_cnt = 0;
...
void spmm_execute_init(uint32_t exec_id) {
    A_nrows = A_shape[0];
    B_ncols = B_shape[1];
    spmm_consume_cnt = 0;
...
void spmm_access_finish(uint32_t id) {
    //DISCONNECT
    dec_close_producer(id);
}

void spmm_execute_finish(uint32_t exec_id) {
    dec_close_consumer(exec_id);
    // LK;printf("spmm_consume_cnt = %d\n", spmm_consume_cnt);ULK;
    // LK;printf("spmm_produce_cnt = %d\n", spmm_produce_cnt);ULK;
}

void spmv_access_init(uint32_t id) {
    #ifdef MAP
...
void spmv_execute_init(uint32_t exec_id) {
    //CONNECT
    #ifdef MAP
...
void spmv_access_finish(uint32_t id) {
    //DISCONNECT
    #ifdef MAP
...
void spmv_execute_finish(uint32_t exec_id) {
    //DISCONNECT
    #ifdef MAP
    ...
    // LK;printf("spmv_consume_cnt = %d\n", spmv_consume_cnt);ULK;
    // LK;printf("spmv_produce_cnt = %d\n", spmv_produce_cnt);ULK;
}
...
void _kernel_(uint32_t id, uint32_t core_num){
    ...
    if (id < NUM_A) {
        ...
        // Finish
        __sync_synchronize;
        spmm_access_finish(id);
        spmv_execute_finish(exec_id);
    } else {
        // spmv access, spmm execute
        // Init
        uint32_t exec_id = id - NUM_A;
        spmv_access_init(id);
        spmm_execute_init(exec_id);
        ...
        // Finish
        __sync_synchronize;
        spmm_execute_finish(exec_id);
        spmv_access_finish(id);
    }
}
